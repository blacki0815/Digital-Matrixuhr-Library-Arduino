...
void setup() {
  for (int i = 0; i < 10; i++) {
    ...
    digitalWrite(zeilen[i], LOW);
  }

  for (int i = 0; i < 11; i++) {
    pinMode(spalten[i], OUTPUT);
    digitalWrite(spalten[i], LOW);
  }
}
...
void loop() {
  for (int r = 0; r < 10; r++) {
    for (int c = 0; c < 11; c++) {
      digitalWrite(zeilen[r], HIGH);
      digitalWrite(spalten[c], HIGH);
      delay(100);
      digitalWrite(zeilen[r], LOW);
      digitalWrite(spalten[c], LOW);
    }
  }
}
